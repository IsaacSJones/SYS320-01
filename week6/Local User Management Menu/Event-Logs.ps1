...
function getRiskUsers($timeBack, $riskLevel){
  ...
  for($i=0; $i -lt $failedlogins.Count; $i++){
    ...
    if ($failedloginsTable.ContainsKey($user)) {
        $failedloginsTable[$user] += 1
    } else {
        $failedloginsTable[$user] = 1
    }
  }
  ...
  $failures = @()
  foreach ($user in $failedloginsTable.GetEnumerator()) {
    if ($user.Value -gt $riskLevel) {
      $failures += [pscustomobject]@{
        "User" = $user.Key;
        "Failed Logins" = $user.Value
      }
    }
  }

  return $failures
} # End of function getRiskUsers
